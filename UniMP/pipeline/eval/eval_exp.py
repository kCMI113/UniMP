...
# from pipeline.eval.rec_metrics import hit_at_k, ndcg_at_k
# from pipeline.train.data import get_data
# from pipeline.train.distributed import init_distributed_device, world_info_from_env
from pipeline.train.train_utils import (  # get_checkpoint,
    AverageMeter,
    get_autocast,
    get_cast_dtype,
)
...
def eval_model_exp(
    ...
    for num_steps, (batch_multi_instruct) in tqdm(
        ...
        real_rates = (
            batch_multi_instruct["net_output"]["output_ratings"][0]
            .cpu()
            .detach()
            .numpy()
        )
    ...
    bleu_all, rouge1_all, rouge2_all, rougeL_all, meteor_all = accelerator.gather(
        (
            torch.tensor(bleu).to(device_id),
            torch.tensor(rouge1).to(device_id),
    ...
    if eval_embed:
        ...
        bertscore_all = accelerator.gather_for_metrics(
            torch.tensor(bertscore).to(device_id)
        )
